...
For a set of DAC fields, the PX1035 diagnostic considers dirty primary and unique keys to be different from regular keys. You can have both regular and dirty primary keys with the same set of fields in a DAC.
...
To fix the issue, you should remove all primary, foreign, or unique key declarations with same set of fields except one.
The code fix for the PX1035 diagnostic removes all primary, foreign, or unique key declarations with the same set of fields except the one for which the code fix was selected.
