# PX1102
This document describes the PX1102 diagnostic.

## Summary

| Code   | Short Description                                                                                                                         | Type    | Code Fix    | 
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------- | ------- | ----------- | 
| PX1102 | The additional delegate parameter of the method with the `PXOverride` attribute must have a name conforming Acumatica naming conventions. | Warning | Available   | 
...
The PX1102 diagnostic ensures that the name of the delegate parameter conforms Acumatica naming conventions. The name of the delegate parameter must start with `base_` followed by the name of the base method being overridden.
For example, if the base method is named `Update`, the delegate parameter must be named `base_Update`. Notice that it is also the name of the method with the `PXOverride` attribute.

## Code Fix
The code fix for the PX1102 diagnostic renames the delegate parameter to `base_<MethodName>` to conform Acumatica naming conventions.
...
The following example demonstrates the correct declaration of the `PXOverride` method with an additional delegate parameter with a correct name.
```cs
...
In the following example, `PXOverride` methods are declared with additional delegate parameters that have incorrect names.
```cs
public class MyGraph : PXGraph<MyGraph>
{
...
public class DerivedGraphExtension : PXGraphExtension<BaseGraphExtension, MyGraph>
{
	/// Overrides <seealso cref="BaseGraphExtension.Add(int, string)"/>
	[PXOverride]
	public int Add(int x, string y, Func<int, string, int> baseFunc)	// Report PX1102
	{
		if (x < 10)
		{
			return x + Convert.ToInt32(y) * 2;
	...
	[PXOverride]
	public void UpdateBalances(Action baseAction)		// Report PX1102
	{
		// Implementation
	}
}
